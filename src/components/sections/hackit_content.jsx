...
// Data
const hackitImages = import.meta.glob('../../images/hackit_images/*', { eager: true, as: 'url' });
import hackit_images from '../../data/hackit_images.json';
const mentorLogos = import.meta.glob('../../images/mentors_images/*', { eager: true, as: 'url' });
...
function HackitContent () {
    ...
    return (
        <div className='flex flex-col items-center gap-8 pb-20 justify-normal' ref={ref}>
            <div className={`hidden lg:inline absolute left-0 w-20 bg-hackit-decoration-left bg-contain bg-left-bottom bg-repeat-y`} style={{ height: `${height}px` }}></div>
            <div className={`hidden lg:inline absolute right-0 w-20 bg-hackit-decoration-right bg-contain bg-right-bottom bg-no-repeat-y`} style={{ height: `${height}px` }}></div>
            ...
            <Section padding='no' className='mx-12 lg:w-1/2 flex flex-col gap-12 self-center'>
                <p className='text-lg'>{i18n.t('hackit.description')}</p>
                <div className='text-left'>{i18n.t('hackit.info', {returnObjects: true}).map((text) => {
                    return (
            ...
            <Section padding='no' className='mx-8 lg:w-2/5 flex flex-col items-center gap-12'>
                ...
                <Marquee gradient='true' direction='left' speed='70'>
                    {hackit_images.map((image, index) => (
                        <div key={index} className='p-0 m-0' >
                            <img src={hackitImages[`../../images/hackit_images/${image.src}`]} alt={i18n.t(image.alt)} className=' h-40' />
                        </div>
                    ))}
                </Marquee>
            </Section>
            ...
            <Section padding='no' className='mx-20 flex flex-col items-center gap-6'>
                ...
                <div className='lg:w-1/2 grid grid-cols-4 gap-12 items-center'>
                    <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                        <img className='h-30' alt={i18n.t('hackit.mentors.microsoft')} src={mentorLogos['../../images/mentors_images/microsoft_logo.png']}/>
                    </motion.div>
                    <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                        <img className='h-30' alt={i18n.t('hackit.mentors.google')} src={mentorLogos['../../images/mentors_images/google_logo.png']}/>
                    </motion.div>
                    <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                        <img className='h-30' alt={i18n.t('hackit.mentors.exxon')} src={mentorLogos['../../images/mentors_images/exxon_logo.png']}/>
                    </motion.div>
                    <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                        <img className='h-30' alt={i18n.t('hackit.mentors.modo')} src={mentorLogos['../../images/mentors_images/modo_logo.png']}/>
                    </motion.div>
                </div>
            </Section>

            <Section padding='no'>
                <LinkButton type='full-primary' smooth href='https://www.hackitba.com.ar/' target='blank' className='text-2xl bg-brand_primary hover:bg-brand_secondary focus:bg-brand_secondary px-8' text={i18n.t('hackit.cta')} />
            </Section>
